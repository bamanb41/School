...
app.get("/all-student-announcements", async (req, res) => {
  try {
    ...
    // Extract announcements with class and section from all student records
    const allAnnouncements = students
      .map((student) => {
        return student.announcement.map((announcement) => {
          return {
            date: announcement.date,
            reason: announcement.reason,
            _id: announcement._id,
            class: student.class,      // Assuming class is a property in the Student model
            section: student.section   // Assuming section is a property in the Student model
          };
        });
      })
      .flat();
...
// Delete announcement for a specific class and section
app.delete("/delete-announcement/:class/:section/:announcementId", async (req, res) => {
  try {
    const { class: section, announcementId } = req.params;
    ...
    // Remove the announcement from each student's notifications array
    students.forEach(async (student) => {
      const indexToRemove = student.announcement.findIndex(
        (announcement) => announcement._id.toString() === announcementId
      );

      if (indexToRemove !== -1) {
        student.announcement.splice(indexToRemove, 1);
        await student.save();
      }
    });

    res.status(200).json({ message: "Announcement deleted for students" });
  } catch (error) {
    console.error(error);
